...
def run_episode(env, agent, rendering=True, max_timesteps=1000):
    ...
    while True:
        ...
        #output = agent(Tensor(state).unsqueeze(1))
        output = agent(Tensor(state).unsqueeze(0).unsqueeze(0))
        a = output_to_action(output)
        # uncomment the lines below if not acceletating initially
        if(step < 10):
...
if __name__ == "__main__":
    ...
    # TODO: load agent
    agent = Model()
    agent.load_state_dict(torch.load("models/agent.ckpt"))
    agent.eval()
